...
from reference_select import load_catalog, pick_references, schedule_kwargs
...
from emotion_schedule import emotion_schedule, to_musemorphose_attrs
...
if __name__ == "__main__":
  ...
  EMOTION = sys.argv[6] if len(sys.argv) > 6 else "Q2"
  catalog = load_catalog('reference_catalog.json')
  pieces = pick_references(EMOTION, catalog, n=n_pieces,
                           min_bars=config['generate']['max_bars'], seed=random.randint(0, 10**6))
  print('[emotion]', EMOTION, '-> reference idxs', pieces)
  ...
  for p in pieces:
    ...
    n_bars = p_latents.size(0)
    sched = emotion_schedule(n_bars=n_bars, **schedule_kwargs(EMOTION))
    rhym_list, poly_list = to_musemorphose_attrs(sched)
    sched_rfreq  = torch.tensor(rhym_list, dtype=torch.long, device=device)
    sched_polyph = torch.tensor(poly_list, dtype=torch.long, device=device)
    assert len(sched_rfreq) == n_bars
    ...
    for samp in range(n_samples_per_piece):
      p_polyph_cls = sched_polyph
      p_rfreq_cls  = sched_rfreq
      ...
      import copy
      if hasattr(orig_tempo, '__len__') and len(orig_tempo) > 0:
          calm_events = copy.deepcopy(orig_tempo)
          for t in calm_events:
              t.tempo = 75.0            # fixed calm tempo
              t.time = 0
          calm_events = calm_events[:1]  
      else:
          calm_events = orig_tempo       # fallback: pass through unchanged
      remi2midi(song, out_file + '.mid', enforce_tempo=True, enforce_tempo_val=calm_events)
